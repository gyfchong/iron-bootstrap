 ...
 * Installation
 *
 * First time instructions, uninstall Grunt globally:
 ...
 *
 * Install the default OOTS packages (will source them from the package.json file); run: 
 * npm install
 *
...
module.exports = function(grunt) {
	...
	* ======================================================================== */
    uglify:
    {
      build:
      {
        ...
        files:
        {
          "<%= pkg.path.deploy %>/<%= pkg.path.lib.js %>/main.min.js" :
          [
    ...
    jshint:
    {
      all:
      [
        "<%= pkg.path.src.js %>/plugins.js",
        "<%= pkg.path.src.js %>/main.js"
      ]
    },
    ...
    copy:
    {
      ...
      scripts:
      {
        ...
        [
          {
            expand: true,
            cwd:    "<%= pkg.path.dev %>/<%= pkg.path.lib.js %>/",
            src:    "**",
            dest:   "<%= pkg.path.enviro %>/<%= pkg.path.lib.js %>/",
            dot:    true
          }
        ]
      },
    ...
    watch:
    {
      ...
      scripts: {
    	...
    	tasks: ["jshint:all","concat:build","uglify:build","copy:scripts"],
  	  },
   ...
   */
  grunt.registerTask("watchSass", ["watch:sass","watch:scripts"]);
  ...
  grunt.registerTask("deploy", [
    "clean:deploy", "copy:deploy", "sass:deploy"
  ]);
